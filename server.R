...
shinyServer(function(input, output, session) {
  ...
  observe({
    ...
    if ( (!is.null(input$file1) |
         ...
         input$protrsubmitButton != 0L ) {
      updateTabsetPanel(
        session, 'protrwebmain', selected = 'Computed Descriptors')
    }
  })

  observe({
    # switch tab
    if ( ((input$text1 != 'Paste FASTA file content here') |
          (input$text2 != 'Paste raw sequence here')) &
         !is.null(input$variable) & input$protrsubmitButton != 0L ) {
      updateTabsetPanel(
        session, 'protrwebmain', selected = 'Computed Descriptors')
    }
  })
  ...
  descTable = reactive({
    ...
    if ( fmtType == 'fasta' &
         iptType == 'pastecontent' &
         (input$text1 != 'Paste FASTA file content here') ) {
  ...
  output$downloadData = downloadHandler(
    filename = function() {
      paste('ProtrWeb-', paste(input$variable, collapse = '-'),
            '-', gsub(' ', '-', gsub(':', '-', Sys.time())),
            '.csv', sep = '')
    },
