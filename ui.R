...
nx.actionButton = function (inputId, label, icon = NULL) {
  if (!is.null(icon))
    buttonContent = list(icon, label) else
      buttonContent = label
    tags$button(
...
shinyUI(fluidPage(
  ...
  sidebarPanel(
    ...
    radioButtons(
      'formatType', 'Step 1. Choose file format',
      c('FASTA' = 'fasta', 'Raw sequence' = 'rawseq')),

    radioButtons(
      'inputType', 'Step 2. Upload or paste?',
      c('Upload file' = 'uploadfile', 'Paste file content' = 'pastecontent')),
    ...
    nx.actionButton(
      'protrsubmitButton',
      'Compute selected descriptors', icon('check'))
